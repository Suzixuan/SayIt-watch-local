...
```bash
SAYIT_OPENAI_API_KEY=<your-key>    # or SAYIT_AZURE_API_KEY
...
├── env.example           # Secrets template; copy to local .env
├── docker-compose.yml
└── LICENSE               # AGPL-3.0
```
...
cd backend
# --no-access-log 关闭访问日志，避免 WebSocket 握手 query 中的鉴权令牌被记录；
# 本地自用建议 --host 127.0.0.1 仅回环监听（默认安全配置）。
