...
describe('isSensitiveKey', () => {
  ...
  it('遮蔽服务器访问令牌（token）', () => {
    expect(isSensitiveKey('serverToken')).toBe(true)
    const result = sanitizeObject({ serverToken: 'tok-1234567890abcdef', theme: 'dark' })
    expect(result.serverToken).toBe('tok***ef')
