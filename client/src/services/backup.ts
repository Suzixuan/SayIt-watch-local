 ...
 *  **默认（includeSecrets=false）不包含任何敏感密钥**；只有用户在导出确认框里
 *  显式勾选「包含密钥」并二次确认后才传 true（Rust 命令层同样默认为 false）。 */
export async function exportFullFile(includeSecrets = false): Promise<ExportResult> {
  const path = await invoke<string>('export_full', { includeSecrets })
  return { canceled: false, filePath: path }
}
