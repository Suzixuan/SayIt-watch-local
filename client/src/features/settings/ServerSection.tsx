...
import { serverFetch, invalidateServerToken, invalidateServerShareMetadata } from '@/services/serverAuth'
...
export default function ServerSection() {
  ...
  useEffect(() => {
    ...
    void getSetting('serverToken', '').then((v) => {
      const value = String(v || '')
      setServerToken(value)
      setSavedServerToken(value)
    })
    void getSetting('serverShareMetadata', false).then((v) => setShareMetadata(Boolean(v)))
  ...
  async function probeHealth(url: string): Promise<{ asr?: boolean; llm?: boolean }> {
    const response = await serverFetch(`${url}/healthz`, { cache: 'no-store' })
    if (!response.ok) throw new Error(`HTTP ${response.status}`)
    return await response.json() as { asr?: boolean; llm?: boolean }
  }

  /** 保存访问令牌并重连。令牌（若有）会随所有 /api 与 /ws 请求发送。 */
  async function handleSaveToken() {
    ...
    try {
      ...
      await setSetting('serverToken', value)
      setSavedServerToken(value)
      invalidateServerToken()
      reconnectProvider()
      setResult({ tone: 'success', message: t('server.tokenSaved') })
    } catch (error) {
      setResult({ tone: 'error', message: t('server.tokenSaveFailed'), detail: String(error) })
    } finally {
      setBusy(false)
    }
  }

  /** 诊断元数据上报授权开关（默认关闭）。开启后，服务器模式的会话才附带这些元数据。 */
  async function handleShareMetadataChange(next: boolean) {
    setShareMetadata(next)
    try {
      await setSetting('serverShareMetadata', next)
    } catch {
      // 落盘失败也把本次内存状态回滚，避免「看起来开了、实际没生效」
      setShareMetadata(!next)
      return
    }
    invalidateServerShareMetadata()
    reconnectProvider()
  }
    ...
    <>
      <Card>
        <CardContent className="p-6">
          ...
          {/* 服务器访问令牌（可选）：服务端设置了 SAYIT_API_TOKEN 时需填写同一值 */}
          <div className="mt-4">
            <div className="mb-1.5 flex items-center gap-2">
              <label htmlFor="server-token" className="text-sm text-muted-foreground">
                {t('server.tokenLabel')}
              </label>
              {serverToken.trim() !== savedServerToken && (
                <span className="rounded-full bg-warning/10 px-2 py-0.5 text-xs font-medium text-warning-strong">
                  {t('server.unsaved')}
                </span>
              )}
            ...
            <div className="flex flex-wrap items-center gap-2">
              <input
                ...
                className="h-9 min-w-[16rem] flex-1 rounded-md border border-input-border bg-input-bg px-3 text-sm transition-colors focus:border-input-focus-border"
              />
              <Button size="sm" className="h-9 shrink-0" onClick={() => void handleSaveToken()} disabled={busy}>
                {t('common.save')}
              </Button>
            </div>
          </div>
          ...
          <div className="mt-4 flex items-start justify-between gap-3">
            <div className="min-w-0">
              <h3 id="server-share-metadata-heading" className="text-sm font-medium">
                {t('server.shareMetadata')}
              </h3>
              <p className="mt-1 text-xs leading-relaxed text-muted-foreground">
                {t('server.shareMetadataDesc')}
              </p>
            </div>
            <Switch
              checked={shareMetadata}
              onChange={() => void handleShareMetadataChange(!shareMetadata)}
              labelledBy="server-share-metadata-heading"
              className="shrink-0"
            />
          </div>
