...
class ServerConfig:
    ...
    # 访问令牌（Bearer）。设置后：/api/* 需 Authorization: Bearer <token>，
    # /ws/* 需 ?token=<token>。留空 = 不鉴权（本地回环部署的默认形态）。
    api_token: str = ""
    # 受信代理名单（IP 或 CIDR）。仅当直连地址命中该名单时，才采信 X-Forwarded-For /
    # X-Real-IP 解析真实客户端 IP；默认空 = 从不信任代理头（本地回环部署的默认形态）。
...
class AdminConfig:
    enabled: bool = True
    username: str = "admin"
    # 默认口令已移除：必须通过 SAYIT_ADMIN_PASSWORD / config.yaml 显式设置，否则视为未启用。
    password: str = ""
...
def load_config(config_path: str | None = None, env_path: str | None = None) -> Config:
    ...
    return Config(
        server=ServerConfig(
            ...
            api_token=_env_str("SAYIT_API_TOKEN", str(server_raw.get("api_token") or "")),
            trusted_proxies=_env_list(
                "SAYIT_TRUSTED_PROXIES",
                [str(x) for x in (server_raw.get("trusted_proxies") or defaults.server.trusted_proxies)],
            ),
        ),
