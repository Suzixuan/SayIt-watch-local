...
export const DEFAULTS: Record<string, unknown> = {
  ...
  // ── 工作模式 ──
  // 本地自用构建：默认「本地模式」（设备端 GGUF 推理，完全离线、数据不出本机）。
  // 云端 ASR/LLM、服务器模式功能完整保留，可在设置中随时切换。
  ...
  // 服务器访问令牌。为空不鉴权；服务端设置了 SAYIT_API_TOKEN 时必须填同一值。
  serverToken: '',
  // 是否向服务器上报诊断元数据（Windows 用户名、主机名、本机 IP、活动进程名/exe 路径等）。
  // 隐私敏感，默认关闭；开启后仅作用于「服务器模式」。
  serverShareMetadata: false,
  ...
  autoCheckUpdate: false, // 本地自用：默认关闭自动更新检查。可选: true | false
